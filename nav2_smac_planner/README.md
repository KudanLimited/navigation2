...
planner_server:
  ros__parameters:
    ...
    GridBased:
      ...
      left_right_change_penalty: 0.0      # For Hybrid nodes: penalty to apply if motion is changing directions left/right in either forwards or reverse, must be >= 0
      forward_reverse_change_penalty: 0.0 # For Hybrid nodes: penalty to apply if motion is changing between forwards and reverse, must be >= 0
